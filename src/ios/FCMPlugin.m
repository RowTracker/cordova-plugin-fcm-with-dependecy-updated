...
- (void)startJsEventBridge:(CDVInvokedUrlCommand *)command {
    NSLog(@"FCM: start Js Event Bridge");
...
- (void)returnTokenOrRetry:(void (^)(NSString* fcmToken))onSuccess {
    ...
    NSLog(@"FCM: FCMToken unavailable, it'll retry in one second");
...
- (void)getAPNSToken:(CDVInvokedUrlCommand *)command  {
    NSLog(@"FCM: get APNS Token");
    [self.commandDelegate runInBackground:^{
        CDVPluginResult* pluginResult = nil;
        NSString* apnsToken = [AppDelegate getAPNSToken];
        NSLog(@"FCM: get APNS Token value: %@", apnsToken);
...
- (void)clearAllNotifications:(CDVInvokedUrlCommand *)command {
  [self.commandDelegate runInBackground:^{
    NSLog(@"FCM: clear all notifications");
...
- (void)requestPushPermission:(CDVInvokedUrlCommand *)command {
    [self.commandDelegate runInBackground:^{
        ...
        NSLog(@"FCM: requestPushPermission { ios9SupportTimeout:%@ ios9SupportInterval:%@ }", ios9SupportTimeout, ios9SupportInterval);
        ...
        [AppDelegate requestPushPermission:^(BOOL pushPermission, NSError* _Nullable error) {
            if(error != nil){
                NSLog(@"FCM: push permission request error: %@", error);
            ...
            NSLog(@"FCM: push permission request result: %@", pushPermission ? @"Yes" : @"No");
...
- (void)getInitialPushPayload:(CDVInvokedUrlCommand *)command {
    NSLog(@"FCM: getInitialPushPayload");
    [self.commandDelegate runInBackground:^{
        ...
        if (error) {
            ...
            NSLog(@"FCM: getInitialPushPayload error: %@", errorMessage);
        ...
        NSLog(@"FCM: getInitialPushPayload value: %@", payloadDictionary);
...
- (void)deleteInstanceId:(CDVInvokedUrlCommand *)command {
    [self.commandDelegate runInBackground:^{
        [AppDelegate deleteInstanceId:^(NSError *error) {
            __block CDVPluginResult *commandResult;
            if(error == nil) {
                NSLog(@"FCM: InstanceID deleted");
                commandResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK];
            } else {
                NSLog(@"FCM: InstanceID deletion error: %@", error);
...
- (void)notifyOfMessage:(NSData *)payload {
    NSLog(@"FCM: notifyOfMessage payload: %@", payload);
...
- (void)notifyFCMTokenRefresh:(NSString *)token {
    NSLog(@"FCM: notifyFCMTokenRefresh token: %@", token);
...
- (void)dispatchJSEvent:(NSString *)eventName withData:(NSString *)jsData {
    if(jsEventBridgeCallbackId == nil) {
        NSLog(@"FCM: dispatchJSEvent: Unable to send event due to unreachable bridge context: %@ with %@", eventName, jsData);
        return;
    }
    NSLog(@"FCM: dispatchJSEvent: %@ with %@", eventName, jsData);
